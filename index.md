...
the rules see, plus a point value used later by the rubric.
...
mode, with optional clamping between zero and the question total). Below
the items are two comment boxes: a public, student-facing comment (which
also marks the question as graded) and private notes that are never
shared with students. Every action autosaves to the project database.
